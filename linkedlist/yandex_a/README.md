...
Пусть $size_i$ \-- размер списка в момент $i$ \-ого запроса.

Ограничения:

- $1 \leq q \leq 5 \cdot 10^3$
- Для запросов первого типа $0 \leq x \leq size_i$ и $1 \leq y \leq 10^6$ .
- Для запросов второго типа $1 \leq x \leq size_i$
- Для запросов третьего типа $1 \leq x \leq size_i$
